...
- Flow: `sendUnder(key,text)` POSTs `{channel, enc}` where `enc = encMsg(key, packMsg(text))` and `packMsg` wraps `{h:myHandle, m:text}` — so the **name rides inside the ciphertext** (the relay still sees nothing; only key holders decrypt the sender). `loadThread(key)` sets `activeKey`/`activeChannel`, shows the thread, `loadRead()`s, `pollThread()`s. `addItem(m, live)` decrypts + `unpackMsg`s (legacy plaintext → no name), skips non-decrypting items; play-only `.fi` shows `[t] name ► PLAY` with the name color-keyed (`handleColor`) and `you` for your own. **Unread = unplayed**: a `NEW` badge until you play it (`playOne` → `markRead`, persisted per channel in `localStorage` `hs_read:<channel>`). A *live* arrival (post-initial-load) from someone else `flash`es + `blip()`s + notifies in the subtitle. PLAY ALL walks a `queue` advanced from `frame()`. Polls the active channel every 4 s (+ manual REFRESH). `#handleIn` "YOU:" sets `myHandle` (random retro default, `localStorage` `hs_handle`).
...
- **Idle line:** off-thread (`view!=='thread'`) the TERMINAL is "alive" — `startIdle()` runs a synthetic carrier + line-noise waterfall (`idleFrame`) and a looping low ambient buffer (`makeIdleBuffer`). The visual runs from load; `idleSound()` brings the ambient up on the first gesture (`ensureAudio`). Purely decorative: no channel polled, nothing leaks. `play()` calls `stopIdle()`; `frame()`/`setView()` restart it when back off-thread.
